...
export class AABB {
  ...
  constructor(options: Partial<IAABBOptions> = {}) {
    /**
   ...
   */
  copy({lowerBound, upperBound}: AABB) {
   ...
   * @param  {AABB} aabb
   */
  extend({lowerBound, upperBound}: AABB) {
   ...
   * @return {Boolean}
   */
  overlaps({lowerBound, upperBound}: AABB) {
...
// Mostly for debugging
   ...
   * @return {Boolean}
   */
  contains({lowerBound, upperBound}: AABB) {
    const l1 = this.lowerBound;
    const u1 = this.upperBound;
    const l2 = lowerBound;
    const u2 = upperBound;
   ...
   * @return {number}
   */
  overlapsRay({_direction, from}: Ray) {
    const t = 0;
